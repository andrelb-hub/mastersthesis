...
% dynare REG_MODEL_v4 savemacro=REG_MODEL_v4_FINAL.mod
...
W1t      ${\hat{W}_{1}}$    (long_name='Wages of region 1')
W2t      ${\hat{W}_{1}}$    (long_name='Wages of region 2')
...
% local variables:
    ...
    thetaC1  % weight of consumption in production of region 1
    thetaC2  % weight of consumption in production of region 2
...
alppha1   ${\alpha_{1}}$    (long_name='capital elasticity of production of region 1')
alppha2   ${\alpha_{2}}$    (long_name='capital elasticity of production of region 2')
...
alppha1  = 0.35  ;  % capital elasticity of production of region 1
alppha2  = 0.25  ;  % capital elasticity of production of region 2
...
rhoA2    = 0.90  ;  % autoregressive parameter of productivity in region 2
...
#W1ss   = (1 - alppha1) * (LAMBDAss * ZA1ss * (alppha1/RKss)^alppha1)^(1/(1 - alppha1));
#W2ss   = (1 - alppha2) * (LAMBDAss * ZA2ss * (alppha2/RKss)^alppha2)^(1/(1 - alppha2));
#a1ss  = ((W1ss^(1 + varphhi) * omega11^(omega11) * (1 - omega11)^(1 - omega11)) / (phhi * Pss * ((1 - alppha1) * LAMBDAss)^varphhi))^(1/siggma);
#a2ss  = ((W2ss^(1 + varphhi) * omega21^(omega21) * (1 - omega21)^(1 - omega21)) / (phhi * Pss * ((1 - alppha2) * LAMBDAss)^varphhi))^(1/siggma);
#b1ss  = dellta * alppha1 * LAMBDAss / RKss;
#b2ss  = dellta * alppha2 * LAMBDAss / RKss;
...
#E1ss  = Pss / (omega11^omega11 * (1 - omega11)^(1 - omega11));
#E2ss  = Pss / (omega21^omega21 * (1 - omega21)^(1 - omega21));
#C11ss = E1ss * C1ss * omega11 / Pss;
#C21ss = E2ss * C2ss * omega21 / Pss;
#C12ss = E1ss * C1ss * (1 - omega11) / Pss;
#C22ss = E2ss * C2ss * (1 - omega21) / Pss;
#K1ss  = alppha1 * Y1ss * LAMBDAss / RKss;
#K2ss  = alppha2 * Y2ss * LAMBDAss / RKss;
#L1ss  = (1 - alppha1) * Y1ss * LAMBDAss / W1ss;
#L2ss  = (1 - alppha2) * Y2ss * LAMBDAss / W2ss;
...
[name='Regional Consumption']
% equation 07 %
C1t = omega11 * C11t + (1 - omega11) * C12t;
C2t = omega21 * C21t + (1 - omega21) * C22t;

[name='Regional Consumption Levels']
% equation 09 %
C11t - C12t = C22t - C21t;

[name='Total Expenses']
% equation 10 %
E1t = omega11 * P1t + (1 - omega11) * P2t;
E2t = omega21 * P1t + (1 - omega21) * P2t;

[name='Labor Supply']
% equation 12 %
varphhi * L1t - (- siggma) * C1t = W1t - E1t;
varphhi * L2t - (- siggma) * C2t = W2t - E2t;

[name='Euler equation for the bonds return']
% equation 14 %
(E1t(+1) - (- siggma) * C1t(+1)) - (E1t - (- siggma) * C1t) = (1 - betta) * Rt;
(E2t(+1) - (- siggma) * C2t(+1)) - (E2t - (- siggma) * C2t) = (1 - betta) * Rt;

[name='Euler equation for the capital return']
% equation 16 %
(E1t(+1) - P1t(+1) - (- siggma) * C1t(+1)) - (E1t - P1t - (- siggma) * C1t) = betta * rK * (RKt(+1) - P1t(+1));
(E2t(+1) - P2t(+1) - (- siggma) * C2t(+1)) - (E2t - P2t - (- siggma) * C2t) = betta * rK * (RKt(+1) - P2t(+1));

[name='Production Function']
% equation 18 %
Y1t = ZA1t + alppha1 * K1t(-1) + (1 - alppha1) * L1t;
Y2t = ZA2t + alppha2 * K2t(-1) + (1 - alppha2) * L2t;

[name='Technical and Economical Marginal Rates of Substitution']
% equation 20 %
K1t(-1) - L1t = W1t - RKt;
K2t(-1) - L2t = W2t - RKt;

% [name='Capital and Labor Levels']
% equation 20 %
% K1t(-1) - L1t = K2t(-1) - L2t;

[name='Marginal Cost']
% equation 22 %
lambda1t = alppha1 * RKt + (1 - alppha1) * W1t - ZA1t - P1t;
lambda2t = alppha2 * RKt + (1 - alppha2) * W2t - ZA2t - P2t;
...
% equation 25 %
  pit = thetaY1 * pi1t + (1 - thetaY1) * pi2t ;
% pit = Pt - Pt(-1);
...
stoch_simul(irf=100, order=1, qz_zero_threshold=1e-20) ZMt ZA1t Rt RKt W1t C1t L1t K1t Y1t I1t P1t pi1t ;

% ZMt ZA1t ZA2t Rt RKt W2t C1t L1t K1t Y1t I1t P1t pi1t C2t L2t K2t Y2t I2t P2t pi2t
