...
% Set up the shock_list and variables
shock_list = {'e_A1', 'e_A2', 'e_M'};
variables = {'ZA1t', 'Rt', 'I1t', 'I2t', 'K1t', 'K2t', 'lambda1t', 'lambda2t', 'Yt', 'Y1t', 'Y2t', 'C1t', 'C2t', 'C11t', 'C21t', 'C12t', 'C22t', 'L1t', 'L2t', 'W1t', 'W2t', 'P1t', 'P2t', 'Q1t', 'Q2t', 'pi1t', 'pi2t', 'pit', 'ZMt', 'ZA2t'};

% Loop through each shock
for i = 1:length(shock_list)
    % Set up Dynare options using set_param_value
    set_param_value('irf', nperiods); % Number of periods for IRFs
    set_param_value('order', 1); % Order of approximation (adjust as needed)
    set_param_value('qz_zero_threshold', 1e-20); % Add the qz_zero_threshold option

    % Run stoch_simul for the current shock
    info = stoch_simul(var_list_, shock_list{i}, options_);

    % Loop through each variable
    for j = 1:length(variables)
        % Extract the IRF data for the current variable
        irf_data = oo_.irfs.(shock_list{i}).(variables{j});

        % Plot the IRF
        figure;
        plot(irf_data);
        title(['IRF to ' shock_list{i} ' for ' variables{j}]);

        % Save the plot as a PNG file
        saveas(gcf, ['IRF_' shock_list{i} '_for_' variables{j} '.png']);
...
ZA1t Rt  I1t I2t K1t  K2t  lambda1t lambda2t Yt 
Y1t  Y2t C1t C2t C11t C21t C12t C22t L1t L2t
W1t  W2t P1t P2t Q1t  Q2t  pi1t pi2t pit
ZMt  ZA2t
