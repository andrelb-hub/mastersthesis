...
\subsubsection*{Household's Utility Maximization Problem}
...
\subsubsection*{Final-Good Firm Maximization Problem}
...
\label{eq:int-good-firm-optimal-price-problem}
	\max_{P_{jt}}: & \quad \E \sum_{s=0}^{\infty} \left\{ \frac{\theta^s}{\prod_{k=0}^{s-1}(1+R_{t+k})} \big[ P_{jt} Y_{j,t+s} - TC_{j,t+s} \big] \right\} \\
...
Substitute \ref{eq:final-good-firm-FOC} in \ref{eq:int-good-firm-optimal-price-problem-2}:
\begin{align}
...
The model is composed by the preview solutions, forming a square system of 16 variables and 16 equations, summarized as follows:
...
\begin{itemize}
	...
	\begin{enumerate}
		...
		\item Market Clearing Condition:
		\begin{align}
			\hat{Y}_t &= \frac{C}{Y}\hat{C}_t + \frac{I}{Y}\hat{I}_t
			\tag{\ref{eq:ll-market-clearing-condition}}
		\end{align}
...
Regions will be identified by the index $i \in \{1,2,\ldots,n\}$. For example, the variable $C\subt{}$ represents the total consumption, while $C\subt[it]$ represents the consumption of region $i$. Without loss of generality, the model will have two regions: the main region and the rest of the country, therefore $i \in \{1,2\}$.
...
\begin{definition}[Medium Scale DSGE Model]
	A Medium Scale DSGE Model has habit formation, capital accumulation, indexation, etc. \cite[p.208]{gali2015monetary}. 
...
%	\cancelto{〈value〉}{〈expression〉} draws a diagonal arrow through the 〈expression〉, pointing to the 〈value〉.
	...
	\begin{align*}
		\sum_{s=0}^{4} \frac{\theta^s}{\prod_{k=0}^{s-1} (1+R_{t+k})}
	\end{align*}
