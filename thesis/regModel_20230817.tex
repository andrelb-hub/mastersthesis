...
%The role of the final-good firm is to aggregate all the varieties produced by the intermediate-good firms, so that the representative consumer can buy only one good $Y_t$, the bundle good. The final-good firm problem is to maximize its profit, considering that its output is the bundle $Y_t$ formed by the continuum of intermediate goods $Y_{\eta jt}$, where $j \in [0,1]$ and $\psi$ is the elasticity of substitution between intermediate goods:
\begin{align}
...
% where $Y_{\eta jt}$ is the output obtained by the production technology level $Z_{A\eta t}$\footnotemark{} that transforms capital $K_{jt}$ and labor $L_{jt}$ in proportions ${\alpha_\eta}$ and $(1-{\alpha_\eta})$, respectively, into intermediate goods. \footnotetext{the production technology level $Z_{A\eta t}$ will be submitted to a productivity shock, detailed in section \ref{sec:productivity shock}.}
...
%	\mathcal{L} = (R_t K_{jt} + W_t L_{jt}) - \Lambda_{\eta t} (Z_{A\eta t} K_{jt}^{\alpha_\eta} L_{jt}^{1-{\alpha_\eta}} - Y_{\eta jt})
%\end{align}
%
%where the Lagrangian multiplier $\Lambda_{\eta t}$ is the marginal cost\footnote{see Lemma \ref{lemma:marginal-cost}}.
%
%\subsubsection*{First Order Conditions}
%
%The first-order conditions are:
\begin{alignat}{2}
%	K_{jt}: \quad & R_t - \Lambda_{\eta t} Z_{A\eta t} {\alpha_\eta} K_{jt}^{{\alpha_\eta}-1} L_{jt}^{1-{\alpha_\eta}} = 0 &&\implies \nonumber \\
	& K_{jt} = {\alpha_\eta} Y_{\eta jt} \frac{\Lambda_{\eta t}}{R_t} \label{eq:int-good-firm-FOC-Kt} \\
%	L_{jt}: \quad & W_t - \Lambda_{\eta t} Z_{A\eta t} K_{jt}^{\alpha_\eta} (1-{\alpha_\eta}) L_{jt}^{-{\alpha_\eta}} = 0 \quad &&\implies \nonumber \\
	& L_{jt} = (1-{\alpha_\eta}) Y_{\eta jt} \frac{\Lambda_{\eta t}}{W_t} \label{eq:int-good-firm-FOC-Lt} \\
%	\Lambda_{\eta t}: \quad & Y_{\eta jt} = Z_{A\eta t} K_{jt}^{\alpha_\eta} L_{jt}^{1-{\alpha_\eta}} \tag{\ref{eq:int-good-firm-production-function}}
\end{alignat}
...
\begin{align}
%	\frac{K_{jt}}{L_{jt}} = \frac{{\alpha_\eta} Y_{\eta jt} \Lambda_{\eta t} /R_t}{(1-{\alpha_\eta}) Y_{\eta jt} \Lambda_{\eta t} /W_t} \implies
...
%	Y_{\eta jt} & = Z_{A\eta t} K_{jt}^{\alpha_\eta} \left[ \left( \frac{1-{\alpha_\eta}}{\alpha_\eta} \right) \frac{R_t K_{jt}}{W_t} \right]^{1-{\alpha_\eta}} &\implies \nonumber \\
...
\begin{alignat}{2}
%	L_{jt} & = \left( \frac{1-{\alpha_\eta}}{\alpha_\eta} \right) \frac{R_t K_{jt}}{W_t} &\implies \nonumber \\
%	L_{jt} & = \left( \frac{1-{\alpha_\eta}}{\alpha_\eta} \right) \frac{R_t}{W_t} \frac{Y_{\eta jt}}{Z_{A\eta t}} \left[ \left( \frac{\alpha_\eta}{1-{\alpha_\eta}} \right) \frac{W_t}{R_t}\right]^{1-{\alpha_\eta}} &\implies \nonumber \\
...
\begin{alignat}{2}
%	TC_{\eta jt} & = W_t L_{jt} + R_t K_{jt} &\implies \nonumber \\
...
%\begin{align}
%	\Lambda_{jt} & = \frac{\partial TC_{\eta jt}}{\partial Y_{\eta jt}} \implies 
%	\Lambda_{jt} = \frac{1}{Z_{A\eta t}} \left( \frac{R_t}{\alpha_\eta} \right)^{\alpha_\eta} \left( \frac{W_t}{1-{\alpha_\eta}} \right)^{1-{\alpha_\eta}} 	\label{eq:int-good-firm-MC}
%\end{align}
...
%\begin{align}
%	\max_{P_{\eta jt}}: & \quad \E \sum_{s=0}^{\infty} \left\{ \frac{\theta^s \left[ P_{\eta jt} Y_{t+s} \left( \frac{P_{t+s}}{P_{\eta jt}} \right)^{\psi} - \Lambda_{\eta t+s} Y_{t+s} \left( \frac{P_{t+s}}{P_{\eta jt}} \right)^{\psi} \right] }{\prod_{k=0}^{s-1}(1+R_{t+k})} \right\} \implies \nonumber 
%	\\
%	\max_{P_{\eta jt}}: & \quad \E \sum_{s=0}^{\infty} \left\{ \frac{\theta^s \left[ P_{\eta jt}^{1-\psi} P_{t+s}^{\psi} Y_{t+s} - P_{\eta jt}^{-\psi} P_{t+s}^{\psi} Y_{t+s} \Lambda_{\eta t+s} \right] }{\prod_{k=0}^{s-1}(1+R_{t+k})} \right\} \nonumber
%\end{align}
%
%%%%%%
%
...
%\begin{align}
%	& \quad \E \sum_{s=0}^{\infty} \left\{ \frac{\theta^s \left[ (1-\psi) P_{\eta jt}^{-\psi} P_{t+s}^{\psi} Y_{t+s} - (-\psi) P_{\eta jt}^{-\psi-1} P_{t+s}^{\psi} Y_{t+s} \Lambda_{\eta t+s} \right] }{\prod_{k=0}^{s-1}(1+R_{t+k})} \right\} = 0 \nonumber
%\end{align}
%
%%%%%%
%
...
%	& \E \sum_{s=0}^{\infty} \left\{ \frac{\theta^s (\psi-1) \left( \frac{P_{t+s}} {P_{\eta jt}} \right)^{\psi} Y_{t+s}} {\prod_{k=0}^{s-1} (1+R_{t+k})} \right\} = \E \sum_{s=0}^{\infty} \left\{ \frac{\theta^s \psi P_{\eta jt}^{-1} \left( \frac{P_{t+s}} {P_{\eta jt}} \right)^{\psi} Y_{t+s} \Lambda_{\eta t+s} }{\prod_{k=0}^{s-1}(1+R_{t+k})} \right\}
%\end{align}
%
%%%%%%
%
...
% where $\pi_{\eta t}$ is the gross inflation rate, defined by:
\begin{align}
...
{\singlespacing
	...
	\begin{itemize}
		...
		\begin{enumerate}
			...
			% \[ \frac{K_{jt}}{L_{jt}} = \left( \frac{\alpha_\eta}{1-{\alpha_\eta}} \right) \frac{W_t}{R_t} \]
