...
\begin{align}
	...
	\max_{C_{\eta t},L_{\eta t},K_{\eta, t+1}}: \quad & U(C_{\eta t},L_{\eta t}) = \E \sum_{t=0}^{\infty} \beta^t \left(\frac{C_{\eta t}^{1-\sigma}}{1-\sigma} - \phi \frac{L_{\eta t}^{1+\varphi}}{1+\varphi} \right) \\
	\label{eq:household-budget-constraint}
	\st: \quad & P_t (C_{\eta t} + I_{\eta t}) = W_t L_{\eta t} + R_t K_{\eta t} + \Pi_{\eta t} \\
...
where $\E$ is the expectation operator, $\beta$ is the intertemporal discount factor, $\sigma$ is the relative risk aversion coefficient, $\phi$ is the relative labor weight in utility, $\varphi$ is the marginal disutility of labor supply. In the budget constraint, $P_t$ is the price level, $I_{\eta t}$ is the investment, $W_t$ is the wage level, $K_{\eta t}$ is the capital stock, $R_t$ is the return on capital, and $\Pi_{\eta t}$ is the firm profit. In the capital accumulation rule, $\delta$ is the capital depreciation rate.

Isolate $I_{\eta t}$ in \ref{eq:law-of-motion-for-capital} and substitute in \ref{eq:household-budget-constraint}:
\begin{align}
	\tag{\ref{eq:law-of-motion-for-capital}}
	& K_{\eta, t+1} = (1-\delta)K_{\eta t} + I_{\eta t} \implies I_{\eta t} = K_{\eta, t+1} - (1-\delta)K_{\eta t} \\
	...
	& P_t (C_{\eta t} + K_{\eta, t+1} - (1-\delta)K_{\eta t}) = W_t L_{\eta t} + R_t K_{\eta t} + \Pi_{\eta t}
\end{align}
...
\begin{multline}
	...
	\left\{ \left( \frac{C_{\eta t}^{1-\sigma}}{1-\sigma} - \phi \frac{L_{\eta t}^{1+\varphi}}{1+\varphi} \right) - \right.
	\\
	\left. -\mu_t \Bigr[ P_t (C_{\eta t} + K_{\eta, t+1} - (1-\delta) K_{\eta t}) - (W_t L_{\eta t} + R_t K_{\eta t} + \Pi_{\eta t}) \Bigl] \right\}
\end{multline}

\subsubsection*{First Order Conditions}

The first order conditions with respect to $C_{\eta t}$, $L_{\eta t}$, $K_{\eta, t+1}$ and $\mu_t$ are:
\begin{align}
	C_{\eta t}: \quad & C_{\eta t}^{-\sigma} -\mu_t P_t = 0 \implies \mu_t = \frac{C_{\eta t}^{-\sigma}}{P_t} \label{eq:household-FOC-Ct} \\
	L_{\eta t}: \quad & -\phi L_{\eta t}^{\varphi} +\mu_t W_t = 0 \implies \mu_t = \frac{\phi L_{\eta t}^{\varphi}}{W_t} \label{eq:household-FOC-Lt} \\
	\begin{split}
	...
	\mu_t: \quad & P_t (C_{\eta t} + K_{\eta, t+1} - (1-\delta)K_{\eta t}) = W_t L_{\eta t} + R_t K_{\eta t} + \Pi_{\eta t} \tag{\ref{eq:household-budget-constraint-2}}
\end{align}
...
\begin{align}
	\max_{Y_{\eta jt}}: &\quad \Pi_{\eta t} = P_t Y_{\eta t} - \int_{0}^{1} P_{\eta jt} Y_{\eta jt} \dif j \label{eq:final-good-firm-max-problem} \\
	\st: & \quad Y_{\eta jt} = \left( \int_{0}^{1} Y_{\eta jt}^{\frac{\psi-1}{\psi}} \dif j \right)^{\frac{\psi}{\psi-1}} \label{eq:final-good-firm-bundle-rule}
\end{align}
...
\footnotetext{the production technology level $Z_{A\eta t}$ will be submitted to a productivity shock, detailed in section \ref{sec:productivity shock}.}
...
\begin{alignat}{2}
	...
	& K_{\eta jt} = {\alpha_\eta} Y_{\eta jt} \frac{\Lambda_{\eta jt}}{R_t} \label{eq:int-good-firm-FOC-Kt} \\
	L_{\eta jt}: \quad & W_t - \Lambda_{\eta jt} Z_{A\eta t} K_{\eta jt}^{\alpha_\eta} (1-{\alpha_\eta}) L_{\eta jt}^{-{\alpha_\eta}} = 0 \quad &&\implies \nonumber \\ 
	& L_{\eta jt} = (1-{\alpha_\eta}) Y_{\eta jt} \frac{\Lambda_{\eta jt}}{W_t} \label{eq:int-good-firm-FOC-Lt} \\
	\Lambda_{\eta jt}: \quad & Y_{\eta jt} = Z_{A\eta t} K_{\eta jt}^{\alpha_\eta} L_{\eta jt}^{1-{\alpha_\eta}} \tag{\ref{eq:int-good-firm-production-function}}
\end{alignat}
...
Substitute $L_{\eta jt}$ from equation \ref{eq:int-good-firm-TMRS} in \ref{eq:int-good-firm-production-function}:
\begin{alignat}{2}
...
\begin{alignat}{2}
	...
	L_{\eta jt} & = \frac{Y_{\eta jt}}{Z_{A\eta t}} \left[ \left( \frac{{\alpha_\eta}}{1-{\alpha_\eta}} \right) \frac{W_t}{R_t}\right]^{-{\alpha_\eta}} \label{eq:int-good-firm-Lt-demand}
\end{alignat}
...
\begin{align}
	...
	\Lambda_{\eta jt} = \frac{1}{Z_{A\eta t}} \left( \frac{R_t}{{\alpha_\eta}} \right)^{{\alpha_\eta}} \left( \frac{W_t}{1-{\alpha_\eta}} \right)^{1-{\alpha_\eta}} \label{eq:int-good-firm-MC}
\end{align}
...
\begin{align}
	...
	&= \E \sum_{s=0}^{\infty} \left\{ \frac{\theta^s \psi P_{\eta jt}^{-1} \left( \frac{P_{t+s}} {P_{\eta jt}} \right)^{\psi} Y_{\eta t+s} \Lambda_{\eta t+s} }{\prod_{k=0}^{s-1}(1+R_{t+k})} \right\} \label{eq:int-good-firm-optimal-price-FOC}
	\end{split}
\end{align}
...
\begin{align}
	...
	P_t & = \left[ \theta P_{\eta, t-1}^{1-\psi} + (1-\theta) P_{\eta t}^{\ast 1-\psi} \right]^\frac{1}{1-\psi} \label{eq:general-price-level}
\end{align}
...
{\singlespacing
	...
	\begin{itemize}
		...
		\begin{enumerate}
			...
			% \[ \frac{K_{\eta jt}}{L_{\eta jt}} = \left( \frac{{\alpha_\eta}}{1-{\alpha_\eta}} \right) \frac{W_t}{R_t} \]
...
\begin{enumerate}
	\item Labor Supply:
	\begin{align}
		\frac{\phi L_{\eta t}^{\varphi}}{C_{\eta t}^{-\sigma}} = \frac{W_t}{P_t} \implies \frac{\phi L_{\eta}^{\varphi}}{C_{\eta}^{-\sigma}} = \frac{W}{P} \label{eq:ss-household-labor-supply}
	\end{align}
	...
	\begin{align}
		...
		1 &= \beta \left[ (1-\delta) + \frac{R}{P} \right] \label{eq:ss-household-euler-equation}
	\end{align}
	
	\item Budget Constraint: 
	\begin{align}
		P_t (C_{\eta t} + I_{\eta t}) &= W_t L_{\eta t} + R_t K_{\eta t} + \Pi_{\eta t} \implies \nonumber \\
		P (C_{\eta} + I_{\eta}) &= W L_{\eta} + R K_{\eta} + \Pi_{\eta} \label{eq:ss-household-budget-constraint}
	\end{align}
	
	\item Law of Motion for Capital:
	\begin{align}
		K_{\eta, t+1} &= (1-\delta)K_{\eta t} + I_{\eta t} \implies K_{\eta} = (1-\delta)K_{\eta} + I_{\eta} \implies \nonumber \\
		I_{\eta} &= \delta K_{\eta} \label{eq:ss-law-of-motion-for-capital}
	\end{align}
	
	\item Bundle Technology:
	\begin{align}
		Y_{\eta t} = \left( \int_{0}^{1} Y_{\eta jt}^{\frac{\psi-1}{\psi}} \dif j \right)^{\frac{\psi}{\psi-1}} \implies %\nonumber \\
		Y_{\eta} = \left( \int_{0}^{1} Y_{\eta j}^{\frac{\psi-1}{\psi}} \dif j \right)^{\frac{\psi}{\psi-1}} \label{eq:ss-final-good-firm-bundle-rule}
	\end{align}
	...
	\begin{align}
		...
		\Lambda_{\eta} &= \frac{1}{Z_{A\eta}} \left( \frac{R}{{\alpha_\eta}} \right)^{{\alpha_\eta}} \left( \frac{W}{1-{\alpha_\eta}} \right)^{1-{\alpha_\eta}} \label{eq:ss-int-good-firm-MC-2}
	\end{align}
	
	\item Production Technology:
	\begin{align}
		Y_{\eta jt} = Z_{A\eta t} K_{\eta jt}^{\alpha_\eta} L_{\eta jt}^{1-{\alpha_\eta}} \implies Y_{\eta j} = Z_{A\eta} K_{\eta j}^{\alpha_\eta} L_{\eta j}^{1-{\alpha_\eta}} \label{eq:ss-int-good-firm-production-function}
	\end{align}
	...
	\begin{alignat}{2}
		...
		P^\ast &= \frac{\psi}{\psi-1} \Lambda_{\eta} \label{eq:ss-int-good-firm-optimal-price-FOC-3}
	\end{alignat}
	
	\item Market Clearing Condition:
	\begin{align}
		Y_t = \sum_{\eta=1}^{n} (C_{\eta t} + I_{\eta t}) \implies Y = \sum_{\eta=1}^{n} (C_{\eta} + I_{\eta}) \label{eq:ss-market-clearing-condition}
	\end{align}
	...
	\begin{alignat}{2}
		...
		\varepsilon_{A\eta} &= 0 \label{eq:ss-productivity-shock}
	\end{alignat}
...
For the steady state solution, all endogenous variables will be determined with respect to the parameters. It's assumed that the productivity and the price level are normalized to one: $\left[ \begin{smallmatrix} P & Z_{A\eta} \end{smallmatrix} \right] = \vec{\bm{1}}$ \footnotemark{}. \footnotetext{where $\vec{\bm{1}}$ is the unit vector.}
...
From \ref{eq:ss-int-good-firm-optimal-price-FOC-3} and \ref{eq:ss-general-price-level}, the marginal cost $\Lambda_{\eta}$ is:
\begin{align}
	P^\ast &= \frac{\psi}{\psi-1} \Lambda_{\eta} \implies \Lambda_{\eta} = P \frac{\psi-1}{\psi} \label{eq:ss-marginal-cost}
\end{align}
...
\begin{alignat}{2}
	...
	W &= (1-{\alpha_\eta}) \left[ \Lambda_{\eta} Z_{A\eta} \left( \frac{{\alpha_\eta}}{R} \right)^{{\alpha_\eta}} \right]^\frac{1}{1-{\alpha_\eta}} &\, \label{eq:ss-nominal-wage}
\end{alignat}
...
Second, log-linearize the left hand side of equation \ref{eq:ll-optimal-price} with respect to \( \Lambda_{\eta t}^\ast, Y_{j,t}, \widetilde{R}_t \):
\begin{alignat}{2}
...
\begin{alignat}{2}
	...
	& \hat{\Lambda}_t = {\alpha_\eta} \hat{R}_t + (1- {\alpha_\eta}) \hat{W}_t - \hat{Z}_{A\eta t} \label{eq:ll-int-good-firm-MC-2}
\end{alignat}
...
\begin{alignat}{2}
	...
	\hat{\lambda}_t &= {\alpha_\eta} \hat{R}_t + (1- {\alpha_\eta}) \hat{W}_t - \hat{Z}_{A\eta t} - \hat{P}_t \label{eq:ll-int-good-firm-MC-3}
\end{alignat}
...
\begin{alignat}{2}
	Y_{\eta jt} &= Z_{A\eta t} K_{\eta jt}^{\alpha_\eta} L_{\eta jt}^{1-{\alpha_\eta}} &\implies \tag{\ref{eq:int-good-firm-production-function}} \\
	...
	\hat{Y}_{jt} &= \hat{Z}_{A\eta t} + {\alpha_\eta} \hat{K}_{jt} + (1-{\alpha_\eta}) \hat{L}_{jt} \label{eq:ll-int-good-firm-production-function}
\end{alignat}
...
\begin{alignat}{2}
	...
	\hat{Y}_t &= \hat{Z}_{A\eta t} + {\alpha_\eta} \int_{0}^{1} \hat{K}_{jt} \dif j + (1-{\alpha_\eta}) \int_{0}^{1} \hat{L}_{jt} \dif j \label{eq:ll-final-good-firm-bundle-rule-2}
\end{alignat}
...
\begin{align}
	...
	\hat{Y}_t &= \hat{Z}_{A\eta t} + {\alpha_\eta} \hat{K}_t + (1-{\alpha_\eta}) \hat{L}_t \label{eq:ll-final-good-firm-bundle-rule-3}
\end{align}
...
\begin{alignat}{2}
	K_{\eta jt} &= {\alpha_\eta} Y_{\eta jt} \frac{\Lambda_{\eta t}}{R_t} &\implies \tag{\ref{eq:int-good-firm-FOC-Kt}} \\
...
\begin{alignat}{2}
	L_{\eta jt} &= (1-{\alpha_\eta}) Y_{\eta jt} \frac{\Lambda_{\eta t}}{W_t} &\implies \tag{\ref{eq:int-good-firm-FOC-Lt}} \\
...
\begin{alignat}{2}
	\ln{Z_{A\eta t}} &= (1-\rho_{A\eta})\ln{Z_{A\eta}} + \rho_{A\eta}\ln{Z_{A\eta,t-1}} + \varepsilon_{A\eta t} &\implies \tag{\ref{eq:productivity-shock}} \\
	\ln{Z_{A\eta}} + \hat{Z}_{A\eta t} &= (1-\rho_{A\eta}) \ln{Z_{A\eta}} + \rho_{A\eta} (\ln{Z_{A\eta}} + \hat{Z}_{A,t-1}) + \varepsilon_{A\eta} \quad &\implies \nonumber \\
	\hat{Z}_{A\eta t} &= \rho_{A\eta} \hat{Z}_{A,t-1} + \varepsilon_{A\eta} \label{eq:ll-productivity-shock}
\end{alignat}
...
{\singlespacing
	...
	\begin{itemize}
		...
		\begin{enumerate}
			...
			\begin{align}
				\hat{\lambda}_t &= {\alpha_\eta} \hat{R}_t + (1- {\alpha_\eta}) \hat{W}_t - \hat{Z}_{A\eta t} - \hat{P}_t \tag{\ref{eq:ll-int-good-firm-MC-3}}
			\end{align}
			
			\item Production Function:
			\begin{align}
				\hat{Y}_t &= \hat{Z}_{A\eta t} + {\alpha_\eta} \hat{K}_t + (1-{\alpha_\eta}) \hat{L}_t \tag{\ref{eq:ll-final-good-firm-bundle-rule-3}}
			\end{align}
			...
			\item Productivity Shock:
			\begin{align}
				\hat{Z}_{A\eta t} &= \rho_{A\eta} \hat{Z}_{A,t-1} + \varepsilon_{A\eta} \tag{\ref{eq:ll-productivity-shock}}
			\end{align}
