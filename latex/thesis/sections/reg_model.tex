...
Consider an economy with price stickiness, following the Calvo Rule \cite{calvo_staggered_1983}: each firm has a probability $(0 < \theta < 1)$ of keeping its price in the next period ($P_{\nu j,t+1} = P_{\nu jt}$), and a probability of $(1 - \theta)$ of setting a new optimal price $P_{\nu jt}^\ast$ that maximizes its profits. Therefore, each firm must take this uncertainty into account when deciding the optimal price: the intertemporal profit flow, given the nominal interest rate $R_{Kt}$ of each period, is calculated considering the probability $\theta$ of keeping the previous price.
...
The model is composed of the preview solutions, forming a square system of 35 variables and 35 equations, summarized as follows:
...
\begin{enumerate}
	\item Labor Supply:
	\begin{align}
		\frac{\phi L_{\eta t}^{\varphi}}{C_{\eta t}^{-\sigma}} = \frac{W_t}{Q_{\eta t}} \implies \frac{\phi L_{\eta}^{\varphi}}{C_{\eta}^{-\sigma}} = \frac{W}{P_\eta} \label{eq:reg-ss-household-labor-supply}
	\end{align}
	...
	\begin{align}
		...
		1 &= \beta \left[ (1-\delta) + \frac{R}{P_{\eta}} \right] \label{eq:reg-ss-household-euler-equation}
	\end{align}
	
	\item Budget Constraint: 
	\begin{align}
		P_{1t} C_{1\eta t} + P_{2t} C_{2\eta t} + P_{\eta t} I_{\eta t} &= W_t L_{\eta t} + R_{Kt} K_{\eta t} + \Pi_{\nu t} \implies \nonumber \\
		P_{1} C_{1\eta} + P_{2} C_{2\eta} + P_{\eta} I_{\eta} &= W L_{\eta} + R K_{\eta} + \Pi_{\nu} \label{eq:reg-ss-household-budget-constraint}
	\end{align}
...
\begin{align}
	...
	1 = \beta \left[ (1-\delta) + \frac{R}{P_{\eta}} \right] \implies 
	R = P_{\eta} \left[ \frac{1}{\beta} - (1-\delta) \right]
\end{align}
...
\begin{comment}
	...
	\begin{alignat}{2}
		\frac{\phi L_{\eta t}^{\varphi}}{C_{\eta t}^{-\sigma}} &= \frac{W_t}{P_{\nu t}} & \implies \tag{\ref{eq:reg-labor-supply}} \\
