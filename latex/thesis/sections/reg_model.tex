...
Rewrite \ref{eq:reg-C1-Cn} in terms of \ref{eq:reg-price-eta-level} and isolate $(\sfrac{\omega_{\eta}}{P_{1t} C_{1\eta t}})$:
	\begin{alignat}{2}
		...
		\frac{\omega_{\eta}}{P_{1t} C_{1\eta t}} = \frac{1}{\te} \label{eq:reg-qc-pc}
	\end{alignat}
...
{\singlespacing
	...
	\begin{itemize}
		...
		%C_{\eta t}, % - acho que esta variável está sobrando... ela não faz parte da maximização...
...
\begin{enumerate}
	...
	\end{split} \tag{\ref{eq:reg-budget-constraint}}
	\\
	& P_{1} C_{1\eta} + P_{2} C_{2\eta} + P_{\eta} I_{\eta} = W L_{\eta} + R_{K} K_{\eta} + R B_{\eta} + \Pi_{\eta} \label{eq:reg-ss-household-budget-constraint}
	\end{align}
...
\end{align}
	...
	\begin{align}
		...
		I_{\eta} &= b_{\eta} Y_{\eta} \quad\text{where:}\quad b_{\eta} = \delta {\alpha_\eta} \frac{\Lambda_{\eta}}{R_K} \label{eq:reg-ss-investment}
	\end{align}
	...
	\begin{align}
		...
		C_{\eta} &= a_{\eta} Y_{\eta}^{\frac{-\varphi}{\sigma}} \quad\text{where:}\quad a_{\eta} = \left[ \frac{W^{1 +\varphi} \omega_{\eta}^{\omega_{\eta}} (1 -\omega_{\eta})^{1 -\omega_{\eta}} }{\phi P_{\eta} \left[ (1 -\alpha_{\eta}) \Lambda_{\eta} \right]^{\varphi} } \right]^{\frac{1}{\sigma}} \label{eq:reg-ss-consumption}
	\end{align}
