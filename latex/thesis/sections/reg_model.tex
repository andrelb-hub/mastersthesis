...
\subsubsection*{Variables at Steady State}
	...
	For the steady state solution, all endogenous variables will be determined with respect to the parameters. It's assumed that one regional price and one productivity levels are normalized to one:\footnote{where $\vec{\bm{1}}$ is the unit vector.}
	\begin{align}
		\langle \begin{smallmatrix} P_{1} & Z_{A1} \end{smallmatrix} \rangle = \vec{\bm{1}} \label{eq:reg-ss-p1-za1}
	\end{align}
