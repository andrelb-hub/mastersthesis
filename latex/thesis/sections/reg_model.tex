...
\begin{align}
	...
	\begin{split} & \frac{ P_{\eta}^{\ast} Y_{\eta j} ( 1 + \hat{P}_{\eta t}^{\ast} )}{1- \theta \beta} + P_{\eta}^{\ast} Y_{\eta j} \E \sum_{s=0}^{\infty} \left\{ \left( \theta \beta \right)^s \left( \hat{Y}_{\eta j, t+s} - \beta \sum_{k=0}^{s-1} \widetilde{R}_{t+k} \right) \right\} = \frac{\psi}{\psi-1} \cdot \frac{Y_{\eta j} \Lambda_{\eta}}{1- \theta \beta} \, + \\ 
		& \qquad + \frac{\psi}{\psi-1} Y_{\eta j} \Lambda_{\eta} \E \sum_{s=0}^{\infty} \left\{ \left( \theta \beta \right)^s \left( \hat{Y}_{\eta j, t+s} + \hat{\Lambda}_{\eta, t+s} - \beta \sum_{k=0}^{s-1} \widetilde{R}_{t+k} \right) \right\} \end{split} \label{eq:reg-ll-optimal-price-3}
\end{align}
...
\begin{align}
	...
	\end{split} \\
	\hat{P}_{\eta t} - \theta \beta \E \hat{P}_{t+1} &= \theta \hat{P}_{\eta, t-1} - \theta^2 \beta \hat{P}_{\eta t} + (1- \theta -\theta \beta + \theta^2 \beta) \hat{P}_{\eta t} + (1-\theta) (1- \theta \beta) \hat{\lambda}_{\eta t} \implies \nonumber \\
...
Log-linearize \ref{eq:reg-capital-euler-equation}:
	\begin{alignat}{2}
		& \frac{ C_{\eta t}^{1-\sigma}}{ \te} P_{\eta t} = \beta \mathbb{E}_t \left\{ \frac{ C_{\eta, t+1}^{1-\sigma}}{ \te[t+1]} [(1-\delta) P_{\eta, t+1} + R_{K,t+1}] \right\} & \implies \tag{\ref{eq:reg-capital-euler-equation}} \\
		& \frac{P_{\eta t} C_{\eta t}^{1-\sigma}}{\te} = \mathbb{E}_t \left\{ \frac{P_{\eta, t+1} C_{\eta, t+1}^{1-\sigma}}{ \te[t+1]} \right\} \left[ \beta (1-\delta) + \beta \mathbb{E}_t \left\{ \frac{R_{K,t+1}}{P_{\eta, t+1}} \right\} \right] & \implies \nonumber \\
		& \frac{P_{\eta} C_{\eta}^{1-\sigma}}{\te[]} (1 +\hat{P}_{\eta t} + (1 -\sigma) \hat{C}_{\eta t} - \hat{\mathscr{E}}_{\eta t}) = & \nonumber \\
		& \qquad = \frac{P_{\eta} C_{\eta}^{1-\sigma}}{\te[]} (1 +\hat{P}_{\eta, t+1} + (1 -\sigma) \hat{C}_{\eta, t+1} - \hat{\mathscr{E}}_{\eta, t+1}) \cdot & \nonumber \\ 
		& \qquad \cdot \left[ \beta (1-\delta) + \beta \frac{R_{K}}{P_{\eta}} \mathbb{E}_t \left\{ 1 + \hat{R}_{K, t+1} -\hat{P}_{\eta, t+1} \right\} \right] & \implies \nonumber \\
		& \frac{1 +\hat{P}_{\eta t} + (1 -\sigma) \hat{C}_{\eta t} - \hat{\mathscr{E}}_{\eta t}}{1 + \E \left\{ \hat{P}_{\eta, t+1} + (1 -\sigma) \hat{C}_{\eta, t+1} - \hat{\mathscr{E}}_{\eta, t+1} \right\}} = & \nonumber \\
		& \qquad = 1 + \beta \frac{R_{K}}{P_{\eta}} \mathbb{E}_t \left\{ \hat{R}_{K, t+1} -\hat{P}_{\eta, t+1} \right\} & \label{eq:reg-ll-capital-euler-equation}
	\end{alignat}
...
\begin{alignat}{2}
	...
	& \Lambda (1+ \hat{\Lambda}_{\eta t}) = \frac{1}{Z_{A\eta}} \left( \frac{R_{K}}{{\alpha_{\eta}}} \right)^{{\alpha_{\eta}}} \left( \frac{W}{1-{\alpha_{\eta}}} \right)^{1-{\alpha_{\eta}}} (1- \hat{Z}_{A\eta t} + {\alpha_{\eta}} \hat{R}_{Kt} + (1- {\alpha_{\eta}}) \hat{W}_{t} ) \implies \nonumber \\
...
\end{comment}
		...
		\begin{alignat}{2}
			& \frac{1 +\hat{P}_{\eta t} + (1 -\sigma) \hat{C}_{\eta t} - \hat{\mathscr{E}}_{\eta t}}{1 + \E \left\{ \hat{P}_{\eta, t+1} + (1 -\sigma) \hat{C}_{\eta, t+1} - \hat{\mathscr{E}}_{\eta, t+1} \right\}} = & \nonumber \\
			& \qquad \qquad = 1 + \beta \frac{R_{K}}{P_{\eta}} \mathbb{E}_t \left\{ \hat{R}_{K, t+1} -\hat{P}_{\eta, t+1} \right\} \tag{\ref{eq:reg-ll-capital-euler-equation}}
		\end{alignat}
