...
The maximization problem with restrictions can be transformed into one without restriction using the Lagrangian function $\mathcal{L}$ formed by \ref{eq:reg-utility-function} and \ref{eq:reg-budget-constraint}:
\begin{align}
	\begin{split}
	& \mathcal{L} = \mathbb{E}_t \sum_{t=0}^{\infty} \beta^t \left\{ \left( \frac{ \left[ C_{\eta 1 t}^{\omega_{\eta 1}} C_{\eta 2 t}^{1-\omega_{\eta 1}} \right]^{1 -\sigma}}{1 -\sigma} - \phi \frac{L_{\eta t}^{1+\varphi}}{1+\varphi} \right) \right. - \\ & \qquad - \mu_t \Big[ P_{1 t} C_{\eta 1 t} + P_{2 t} C_{\eta 2 t} + B_{\eta t} - \\ & \qquad \qquad \left. - ( W_t L_{\eta t} + (1 + R_{t-1}) B_{\eta,t-1} + \Pi_{\eta t} ) \Big] \right\} \label{eq:reg-household-lagrangian}
	\end{split}
\end{align}
...
Define the regional consumer price index $P_{C \eta t}$: \colorbox{cyan}{version 04}
\begin{align}
	P_{C \eta t} C_{\eta t} = P_{1 t} C_{\eta 1 t} + P_{2 t} C_{\eta 2 t} \implies P_{C \eta t} = \frac{P_{1 t} C_{\eta 1 t} + P_{2 t} C_{\eta 2 t}}{C_{\eta t}} \label{eq:reg-total-expense}
\end{align}

Substitute \ref{eq:reg-Cn-Cv} and \ref{eq:reg-Cn-Cn} in \ref{eq:reg-total-expense}:
\begin{align}
	P_{C \eta t} C_{\eta t} &= P_{1 t} C_{\eta t} \left[ \frac{P_{1 t}}{P_{2 t}} \cdot \frac{1 -\omega_{\eta 1}}{\omega_{\eta 1}} \right]^{\omega_{\eta 1} -1} + P_{2 t} C_{\eta t} \left[ \frac{P_{1 t}}{P_{2 t}} \cdot \frac{1 -\omega_{\eta 1}}{\omega_{\eta 1}} \right]^{\omega_{\eta 1}} \implies \nonumber \\
	P_{C \eta t} &= \left[ \frac{P_{1 t}}{\omega_{\eta 1}} \right]^{\omega_{\eta 1}} \left[ \frac{P_{2 t}}{1 -\omega_{\eta 1}} \right]^{1 -\omega_{\eta 1}} \label{eq:reg-total-expense-level}
\end{align}
...
\begin{align}
	P_{\eta}^{\ast} &= P_{\eta} \implies \langle \begin{matrix} P_{1}^{\ast} & P_{2}^{\ast} \end{matrix} \rangle = \langle \begin{matrix} P_{1} & P_{2} \end{matrix} \rangle \tag{\ref{eq:reg-ss-general-price-level}} \implies \\
	P_{1} &= P_{2} = P_{1}^{\ast} = P_{2}^{\ast} \label{eq:reg-ss-p1-p2-b}
\end{align}
...
Substitute \ref{eq:reg-ss-p1-p2} and then \ref{eq:reg-ss-total-yn} in \ref{eq:ss-national-price-level} for the national price level:
	\begin{align}
		...
		P_{} &= P_{1} = P_{2} = P_{1}^{\ast} = P_{2}^{\ast} \label{eq:reg-ss-P1-P2-P}
	\end{align}
...
Substitute \ref{eq:reg-ss-P1-P2-P} in \ref{eq:reg-ss-int-good-firm-optimal-price-FOC-3} for the marginal cost $\Lambda_{\eta}$:
\begin{align}
