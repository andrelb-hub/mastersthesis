...
\subsubsection*{Solutions}
	...
	Divide \ref{eq_v6:reg-C-eta1t} by \ref{eq_v6:reg-C-eta2t}:
	\begin{align}
		\frac{C_{\eta 1 t}}{C_{\eta 2 t}} &= \frac{\omega_{\eta 1} Q_{\eta t} C_{\eta t} / P_{1t}}{(1 - \omega_{\eta 1}) Q_{\eta t} C_{\eta t} / P_{2t}} \implies \nonumber \\
		\frac{C_{\eta 1 t}}{C_{\eta 2 t}} &= \frac{\omega_{\eta 1} P_{2t}}{(1 - \omega_{\eta 1}) P_{1t}} \label{eq_v6:reg-C-eta-12-t}
	\end{align}
	
	Divide \ref{eq_v6:reg-C-eta-12-t} of region 1 by region 2:
	\begin{align}
		\frac{C_{11t}/C_{12t}}{C_{21t}/C_{22t}} &= \frac{\omega_{11} P_{2t} / (1 - \omega_{11}) P_{1t}}{\omega_{21} P_{2t} / (1 - \omega_{21}) P_{1t}} \implies \nonumber \\
		\frac{C_{11t} C_{22t}}{C_{21t} C_{12t}} &= 
		\frac{\omega_{11} (1 - \omega_{21})}{\omega_{21} (1 - \omega_{11})} \label{eq_v6:reg-C-12-12-t}
	\end{align}
...
Substitute \ref{eq_v6:reg-int-good-firm-profit} in \ref{eq_v6:reg-int-good-firm-profit-total}:
\begin{align}
	\Pi_{\eta t} &= \int_{0}^{1} P_{\eta t} Y_{\eta jt} - W_{\eta t} L_{\eta jt} \dif j \label{eq_v6:reg-int-good-firm-profit-total-2}
\end{align}
...
\begin{comment}
	\begin{align}
		...
		P_{t} &= \theta_{P1} P_{1t} + (1 -\theta_{P1}) P_{2t} %\label{eq_v6:reg-national-price-level}
	\end{align}
...
\subsubsection{Equilibrium Conditions}
	...
	\begin{align}
		...
		& \quad \text{where:} \quad Y_{\eta t} = C_{1 \eta t} + C_{2 \eta t} \label{eq_v6:reg-regional-demand} \\
...
The model is composed of the preview solutions, forming a square system of 36 variables and 36 equations, summarized as follows:
...
{\singlespacing
	...
	\begin{itemize}
		...
		\item from the household problem: $\langle \begin{matrix} C_{\eta t} & L_{\eta t} & B_{\eta t} & C_{\eta 1t} & C_{\eta 2t} & Q_{\eta t} \end{matrix} \rangle$;
		
		\item from the final-goods firm problem: $\langle \begin{matrix} Y_{\eta jt} & Y_{\eta t} & P_{\eta t} \end{matrix} \rangle$;

		\item from the intermediate-goods firm problems: $\langle \begin{matrix} L_{\eta jt} & P_{\eta t}^{\ast} & \Pi_{\eta} \end{matrix} \rangle$;
		
		\item from the monetary policy: $\langle \begin{matrix} R_{t} & \pi_{t} & Y_{t} \end{matrix} \rangle$;
		
		% \item from the market clearing condition: $\langle \begin{matrix} I_{\eta t} \end{matrix} \rangle$;
		
		\item prices: $\langle \begin{matrix} W_{\eta t} & \Lambda_{\eta t} & P_{t} & \pi_{\eta t} \end{matrix} \rangle$;
		
		\item shocks: $\langle \begin{matrix} Z_{A\eta t} & Z_{Mt} \end{matrix} \rangle$.
	...
	\begin{enumerate}
		...
		\item Region 1 Consumption Weights:
		\begin{align}
			\frac{C_{11t}}{C_{12t}} &= \frac{\omega_{11} P_{2t}}{(1 - \omega_{11}) P_{1t}} \tag{\ref{eq_v6:reg-C-eta-12-t}}
		\end{align}
		...
		\begin{align}
		& Q_{\eta t} C_{\eta t} + B_{\eta t} = W_{\eta t} L_{\eta t} + (1 + R_{t-1}) B_{\eta,t-1} + \Pi_{\eta t} \tag{\ref{eq_v6:reg-budget-constraint}}
		\end{align}
		...
		\item Regional Euler equation for consumption:
		\begin{align}
		...
		\begin{align}
			P_{t} &= \frac{P_{1t} Y_{1t} + P_{2t} Y_{2t}}{Y_{t}} \tag{\ref{eq_v6:reg-national-price-level}} % P_{t} = \theta_{P1} P_{1t} + (1 -\theta_{P1}) P_{2t}
		\end{align}
...
\begin{enumerate}
	...
	\item Region 1 Consumption Weights:
	\begin{align}
		\frac{C_{11}}{C_{12}} &= \frac{\omega_{11} P_{2}}{(1 - \omega_{11}) P_{1}} \label{eq_v6:reg-ss-C-eta-12-t}
	\end{align}
	
	\item Regional Consumption Weights:
	\begin{align}
		\frac{C_{11} C_{22}}{C_{21} C_{12}} &= \frac{\omega_{11} (1 - \omega_{21})}{\omega_{21} (1 - \omega_{11})} \label{eq_v6:reg-ss-C-12-12-t}
	\end{align}
	...
	\item Budget Constraint:
	\begin{align}
		& Q_{\eta} C_{\eta} + B_{\eta} = W_{\eta} L_{\eta} + (1 + R_{}) B_{\eta} + \Pi_{\eta} \implies \nonumber \\
		& Q_{\eta} C_{\eta} = W_{\eta} L_{\eta} + R_{} B_{\eta} + \Pi_{\eta} \label{eq_v6:reg-ss-budget-constraint}
	\end{align}
	...
	\item Monetary Policy:
	\begin{align}
		\frac{R_{}}{R_{}} &= \left( \frac{R_{}}{R_{}} \right)^{\gamma_R} \left[ \left( \frac{\pi_{}}{\pi_{}} \right)^{\gamma_\pi} \left( \frac{Y_{}}{Y_{}} \right)^{\gamma_Y} \right]^{1-\gamma_R} Z_{M} \implies \nonumber \\
		Z_{M} &= 1 \label{eq_v6:reg-ss-monetary-policy}
	\end{align}
	
	\item National Gross Inflation Rate:
	\begin{align}
		\pi_{} = \frac{P_{}}{P_{}} = 1 \label{eq_v6:reg-ss-gross-inflation-rate}
	\end{align}
	
	\item National Price Level:
	\begin{align}
		P_{} &= \frac{P_{1} Y_{1} + P_{2} Y_{2}}{Y_{}} \label{eq_v6:reg-ss-national-price-level} % P_{t} = \theta_{P1} P_{1} + (1 -\theta_{P1}) P_{2}
	\end{align}
...
\subsubsection{Variables at Steady State}
	...
	\begin{align}
		\langle \begin{matrix} P_{1} & P_{2} & Z_{A1} & Z_{A2} \end{matrix} \rangle = \vec{\bm{1}} \label{eq_v6:reg-ss-p1-za1} \\
...
\begin{align}
	P_{\eta}^{\ast} = P_{\eta} \implies \langle \begin{matrix} P_{1}^{\ast} & P_{2}^{\ast} \end{matrix} \rangle = \langle \begin{matrix} P_{1} & P_{2} \end{matrix} \rangle = \langle \begin{matrix} P_{1} & P_{1} \end{matrix} \rangle \label{eq_v6:reg-ss-p1-p2}
\end{align}
...
Substitute \ref{eq_v6:reg-ss-p1-p2} and \ref{eq_v6:reg-ss-market-clearing-condition-Yt} in \ref{eq_v6:reg-ss-national-price-level} for the national price level:
	\begin{align}
		P_{} &= \frac{P_{1} Y_{1} + P_{2} Y_{2}}{Y_{}} = \frac{P_{1} Y_{1} + P_{1} Y_{2}}{Y_{}} \tag{\ref{eq_v6:reg-ss-national-price-level}} \implies \nonumber \\
		P_{} &= P_{1} = P_{2} = P_{2}^{\ast} = P_{1}^{\ast} \label{eq_v6:reg-ss-p1-p2-p}
	\end{align}

Substitute \ref{eq_v6:reg-ss-p1-p2-p} in \ref{eq_v6:reg-ss-total-expense-level} for the price composition of consumption bundle $Q_{\eta}$:
\begin{align}
	...
	Q_{1} &= \frac{1}{\omega_{11}^{\omega_{11}} (1 - \omega_{11})^{1 - \omega_{11}}} \label{eq_v6:reg-ss-qn}
\end{align}

Substitute \ref{eq_v6:reg-ss-p1-p2-p} in \ref{eq_v6:reg-ss-int-good-firm-optimal-price-FOC-3} for the marginal cost $\Lambda_{\eta}$:
\begin{align}
...
Substitute \ref{eq_v6:reg-ss-p1-za1}, \ref{eq_v6:reg-ss-za2-0} and \ref{eq_v6:reg-ss-marginal-cost} in \ref{eq_v6:reg-ss-int-good-firm-FOC-Lt} for the nominal wage $W_{\eta}$:
\begin{alignat}{2}
	\Lambda_{\eta} &= \frac{W_{\eta}}{Z_{A\eta}} \tag{\ref{eq_v6:reg-ss-int-good-firm-FOC-Lt}} \\
	W_{\eta} &= \Lambda_{} Z_{A\eta} \label{eq_v6:reg-ss-nominal-wage}
\end{alignat}
...
\end{comment}
	...
	Isolate $C_{\eta}$ in \ref{eq_v6:reg-ss-labor-supply} and then substitute \ref{eq_v6:reg-ss-qn} and \ref{eq_v6:reg-ss-p1-za1}:
	\begin{align}
		...
		& \text{where:} \quad a_{\eta} = \left[ \frac{W_{\eta}}{\phi Z_{A\eta}^{\varphi} Q_{\eta}} \right]^{\frac{1}{\sigma}} \label{eq_v6:reg-ss-a-eta}
	\end{align}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Isolate $C_{11}$ in \ref{eq_v6:reg-ss-C-eta-12-t} and substitute in \ref{eq_v6:reg-ss-consumption-aggregation}:
\begin{align}
	C_{11} &= C_{12} \frac{\omega_{11} P_{2}}{(1 - \omega_{11}) P_{1}} \tag{\ref{eq_v6:reg-ss-C-eta-12-t}} \\
	C_{1} &= C_{11}^{\omega_{11}} C_{12}^{1-\omega_{11}} = \left[ C_{12} \frac{\omega_{11} P_{2}}{(1 - \omega_{11}) P_{1}} \right]^{\omega_{11}} C_{12}^{1-\omega_{11}} \tag{\ref{eq_v6:reg-ss-consumption-aggregation}} \implies \\
	C_{12} &= C_{1} b_{1}^{\omega_{11}} \label{eq_v6:reg-ss-C12} \\
	& \text{where: } b_{1} = \left( \frac{1 - \omega_{11}}{\omega_{11}} \right) \label{eq_v6:reg-ss-b1}
\end{align}

Substitute \ref{eq_v6:reg-ss-C12} in \ref{eq_v6:reg-ss-C-eta-12-t}:
\begin{align}
	C_{11} &= C_{12} \frac{\omega_{11} P_{2}}{(1 - \omega_{11}) P_{1}} = C_{1} \left( \frac{1 - \omega_{11}}{\omega_{11}} \right)^{\omega_{11}} \frac{\omega_{11} P_{2}}{(1 - \omega_{11}) P_{1}} \tag{\ref{eq_v6:reg-ss-C-eta-12-t}} \implies \\
	C_{11} &= C_{1} b_{1}^{\omega_{11} - 1} \label{eq_v6:reg-ss-C11}
\end{align}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Substitute \ref{eq_v6:reg-ss-C12} and \ref{eq_v6:reg-ss-C11} in \ref{eq_v6:reg-ss-C-12-12-t}:
\begin{align}
	\frac{C_{11} C_{22}}{C_{21} C_{12}} &= \frac{\omega_{11} (1 - \omega_{21})}{\omega_{21} (1 - \omega_{11})} \tag{\ref{eq_v6:reg-ss-C-12-12-t}} \implies \\
	C_{22} &= C_{21} \left( \frac{1 - \omega_{21}}{\omega_{21}} \right) \label{eq_v6:reg-ss-C22}
\end{align}

Substitute \ref{eq_v6:reg-ss-C22} in \ref{eq_v6:reg-ss-consumption-aggregation}:
\begin{align}
	C_{2} &= C_{21}^{\omega_{21}} C_{22}^{1-\omega_{21}} = C_{21}^{\omega_{21}} \left[ C_{21} \left( \frac{1 - \omega_{21}}{\omega_{21}} \right) \right]^{1-\omega_{21}} \tag{\ref{eq_v6:reg-ss-consumption-aggregation}} \implies \\
	C_{21} &= C_{2} b_{2}^{\omega_{21} - 1} \label{eq_v6:reg-ss-C21} \\
	& \text{where: } b_{2} = \left( \frac{1 - \omega_{21}}{\omega_{21}} \right) \label{eq_v6:reg-ss-b2}
\end{align}

Substitute \ref{eq_v6:reg-ss-C21} in \ref{eq_v6:reg-ss-C22}:
\begin{align}
	C_{22} &= C_{21} \left( \frac{1 - \omega_{21}}{\omega_{11}} \right) = C_{2} \left( \frac{\omega_{21}}{1 - \omega_{21}} \right)^{1 - \omega_{21}} \left( \frac{1 - \omega_{21}}{\omega_{11}} \right) \tag{\ref{eq_v6:reg-ss-C22}} \implies \\
	C_{22} &= C_{2} b_{2}^{\omega_{21}} \label{eq_v6:reg-ss-C22-b}
\end{align}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Substitute \ref{eq_v6:reg-ss-C11}, \ref{eq_v6:reg-ss-C21} and \ref{eq_v6:reg-ss-consumption} in \ref{eq_v6:reg-ss-regional-demand}:
	\begin{alignat}{2}
		Y_{1} &= C_{11} + C_{21} &\implies \tag{\ref{eq_v6:reg-ss-regional-demand}} \\
		Y_{1} &= C_{1} b_{1}^{\omega_{11} - 1} + C_{2} b_{2}^{\omega_{21} - 1} &\implies \nonumber \\
		a_{2} b_{2}^{\omega_{21}} Y_{2}^{\sfrac{-\varphi}{\sigma}} &= b_{2} \left( Y_{1} - a_{1} b_{1}^{\omega_{11} - 1} Y_{1}^{\sfrac{-\varphi}{\sigma}} \right) \\
		Y_{2} &= \left[ \frac{b_{2} \left( Y_{1} - a_{1} b_{1}^{\omega_{11} - 1} Y_{1}^{\sfrac{-\varphi}{\sigma}} \right) }{a_{2} b_{2}^{\omega_{21}}} \right]^{\sfrac{-\sigma}{\varphi}} \label{eq_v6:reg-ss-total-yn-2}
	\end{alignat}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

	The result of \ref{eq_v6:reg-ss-total-yn-2} determines $Y, I_{\eta}, C_{\eta}, \te[],  C_{\eta 1}, C_{\eta 2}, K_{\eta}, L_{\eta}$ in \ref{eq_v6:reg-ss-market-clearing-condition}, \ref{eq_v6:reg-ss-investment}, \ref{eq_v6:reg-ss-consumption}, \ref{eq_v6:reg-ss-qn},  \ref{eq_v6:reg-ss-C1-Cn-3}, \ref{eq_v6:reg-ss-C12}, \ref{eq_v6:reg-ss-int-good-firm-FOC-Kt-2} and \ref{eq_v6:reg-ss-int-good-firm-FOC-Lt-2}, respectively.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\begin{align}
		...
		\\
		\te[] &= \frac{P_{} C_{\eta}}{\omega_{\eta 1}^{\omega_{\eta 1}} (1 -\omega_{\eta 1})^{1 -\omega_{\eta 1}}} \tag{\ref{eq_v6:reg-ss-qn}}
		\\
		C_{\eta 1} &= C_{\eta} \left[ \frac{\omega_{\eta 1} P_{2}}{(1 - \omega_{\eta 1}) P_{1}} \right]^{1-\omega_{\eta 1}} \tag{\ref{eq_v6:reg-ss-C1-Cn-3}}
		\\
		C_{\eta 2} &= C_{\eta} \left[ \frac{(1 - \omega_{\eta 1}) P_{1}}{\omega_{\eta 1} P_{2}} \right]^{\omega_{\eta 1}} \tag{\ref{eq_v6:reg-ss-C12}}
		\\
...
Define the regional weights $\langle \begin{matrix} \theta_{Y1} & (1-\theta_{Y1}) \end{matrix} \rangle$ in the production total:
\begin{align}
...
\begin{itemize}
	...
	Real Variables: $\langle \begin{matrix} \hat{C}_{\eta} & \hat{C}_{\eta 1} & \hat{C}_{\eta 2} & \hat{L}_{\eta} & \hat{K}_{\eta} & \hat{Y} & \hat{Y}_{\eta} & \hat{I}_{\eta} & \hat{Z}_{A\eta} \end{matrix} \rangle$;
	
	Nominal Variables: $\langle \begin{matrix} \hat{\mathscr{E}}_{\eta} & \hat{P} & \hat{P}_{\eta} & \hat{R} & \hat{R}_{K} & \hat{W} & \hat{\pi} & \hat{\pi}_{\eta} & \hat{\lambda}_{\eta} & \hat{Z}_M \end{matrix} \rangle$.
