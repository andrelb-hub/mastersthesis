...
\subsection{Regional New Keynesian Model}\label{sec_v6:nk-model}
	...
	Each intermediate-goods firm produces a single intermediate-good, all exhibiting imperfect substitution, thus operating in monopolistic competition. Intermediate-goods firms have two problems to solve: minimize costs subject to production level and choose an optimal price to maximize the intertemporal profit flow.
...
The household problem can be divided into two components: first, minimizing consumption costs, and then maximizing utility subject to a budget constraint.

\subsubsection*{Cost Minimization Problem}

Considering that the representative household must decide to consume goods from both regions, there must be a consumption bundle index $C_{\eta t}$ and a consumption price index $Q_{\eta t}$ the minimizes the total consumption cost $Q_{\eta t} C_{\eta t}$, as demonstrated by \textcite[p.424]{walsh_monetary_2017}:
	\begin{align}
...
\subsubsection*{Utility Maximization Problem}
	...
	where $\E$ is the expectation operator, $\beta$ is the intertemporal discount factor, $\sigma$ is the relative risk aversion coefficient, $\phi$ is the relative labor weight in utility, $\varphi$ is the marginal disutility of labor supply. In the budget constraint, $P_{1t}$ and $P_{2t}$ are the prices of goods $1$ and $2$, respectively, $C_{\eta 1 t}$ and $C_{\eta 2 t}$ are the goods produced in region $1$ and $2$, respectively, and consumed in region $\eta$, 
	% $I_{\eta t}$ is the investment, 
	$B_{\eta t}$ are the bonds, $W_{\eta t}$ is the wage level,
	% $K_{\eta t}$ is the capital, $R_{Kt}$ is the return on capital,
	$R_t$ is the return on bonds (which is also the nominal interest rate of the economy) and $\Pi_{\eta t}$ is the firm profit. 
	% In the capital accumulation rule, $\delta$ is the capital depreciation rate. In the consumption aggregation, ${\omega_{\eta 1}}$ and ${1 - \omega_{\eta 1}}$ are the weights of goods $C_{\eta 1 t}$ and $C_{\eta 2 t}$, respectively, in the consumption bundle $C_{\eta t}$.
...
\subsubsection{Equilibrium Conditions}
	...
	\begin{align}
		...
		& \quad \text{where:} \quad P_{\eta t} Y_{\eta t} = P_{1t} C_{1 \eta t} + P_{2t} C_{2 \eta t} + B_{\eta t} = Q_{\eta t} C_{\eta t} + B_{\eta t} \label{eq_v6:reg-regional-demand} \\
...
\end{comment}
		...
		\begin{align}
			P_{\eta t} Y_{\eta t} = Q_{\eta t} C_{\eta t} + B_{\eta t} \tag{\ref{eq_v6:reg-regional-demand}}
		\end{align}
...
\begin{enumerate}
	...
	\item Regional Market Clearing Condition: 
	\begin{align}
		P_{\eta} Y_{\eta} = Q_{\eta} C_{\eta} + B_{\eta} \label{eq_v6:reg-ss-regional-demand}
	\end{align}
...
Substitute \ref{eq_v6:reg-ss-C12} and \ref{eq_v6:reg-ss-budget-constraint-3} in \ref{eq_v6:reg-ss-C-eta-12-t}:
\begin{align}
...
Substitute \ref{eq_v6:reg-ss-consumption} and \ref{eq_v6:reg-ss-budget-constraint-3} in \ref{eq_v6:reg-ss-regional-demand}:
	\begin{alignat}{2}
		P_{\eta} Y_{\eta} &= Q_{\eta} C_{\eta} + B_{\eta} &\implies \tag{\ref{eq_v6:reg-ss-regional-demand}} \\
...
\begin{align}
	& \lambda_{\eta t} = \frac{\Lambda_{\eta t}}{P_{\eta t}} \implies \Lambda_{\eta t} = P_{\eta t} \lambda_{\eta t} \implies \label{eq_v6:reg-real-marginal-cost} \\
...
Substitute \ref{eq_v6:reg-ll-market-clearing-condition-Lt} in \ref{eq_v6:reg-ll-final-goods-firm-bundle-rule-2}: % and \ref{eq_v6:reg-ll-capital-clearing-condition} 
\begin{align}
...
\begin{align}
	P_{\eta t} Y_{\eta t} &= Q_{\eta t} C_{\eta t} + B_{\eta t} \tag{\ref{eq_v6:reg-regional-demand}} \implies \\
	\hat{P}_{\eta t} + \hat{Y}_{\eta t} &= \frac{Q_{\eta} C_{\eta}}{P_{\eta} Y_{\eta}} (\hat{Q}_{1t} + \hat{C}_{\eta t}) + \frac{B_{\eta}}{P_{\eta} Y_{\eta}} \hat{B}_{\eta t} \label{eq_v6:reg-regional-demand-ll}
\end{align}
...
\begin{align}
	\left\langle \begin{matrix} \theta_{B \eta} & (1-\theta_{B \eta}) \end{matrix} \right\rangle \coloneq \left\langle \begin{matrix} \frac{B_{\eta}}{P_{\eta} Y_{\eta}} & \frac{Q_{\eta} C_{\eta}}{P_{\eta} Y_{\eta}} \end{matrix} \right\rangle \label{eq_v6:reg-ss-Q-weight-in-Y} \end{align}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Substitute \ref{eq_v6:reg-ss-Q-weight-in-Y} in \ref{eq_v6:reg-regional-demand-ll}:
\begin{align}
	\hat{P}_{\eta t} + \hat{Y}_{\eta t} &= \theta_{B\eta} (\hat{Q}_{1t} + \hat{C}_{\eta t}) + (1-\theta_{B\eta}) \hat{B}_{\eta t} \label{eq_v6:reg-regional-demand-ll-2}
\end{align}
...
\begin{align}
	...
	\hat{P}_{\eta t} + \hat{Y}_{\eta t} &= \hat{Q}_{1t} + \hat{C}_{\eta t} \label{eq_v6:reg-budget-constraint-3}
\end{align}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Substitute \ref{eq_v6:reg-regional-demand-ll} in \ref{eq_v6:reg-regional-demand-ll-2}:
\begin{align}
	\hat{Q}_{1t} = \hat{B}_{\eta t} - \hat{C}_{\eta t} \label{eq_v6:reg-regional-demand-ll-3}
\end{align}

Equation \ref{eq_v6:reg-regional-demand-ll-3} demonstrates that the variation on the price level composition will affect the variation of consumption levels.
...
\begin{itemize}
	...
	\item Regional Gross Inflation Rate
	\begin{align}
		\hat{\pi}_{1t} &= \hat{P}_{1t} - \hat{P}_{1, t-1} \tag{\ref{eq_v6:level-dev-regional-inflation}a} \\
		\hat{\pi}_{2t} &= \hat{P}_{2t} - \hat{P}_{2, t-1} \tag{\ref{eq_v6:level-dev-regional-inflation}b}
	\end{align}
	
	\item New Keynesian Phillips Curve
	\begin{align}
		\hat{\pi}_{1 t} &= \beta \E \hat{\pi}_{1, t+1} + \frac{(1-\theta) (1- \theta \beta)}{\theta} \hat{\lambda}_{1 t} \tag{\ref{eq_v6:reg-nk-phillips-curve-mc}a} \\
		\hat{\pi}_{2 t} &= \beta \E \hat{\pi}_{2, t+1} + \frac{(1-\theta) (1- \theta \beta)}{\theta} \hat{\lambda}_{2 t} \tag{\ref{eq_v6:reg-nk-phillips-curve-mc}b}
	\end{align}
	
	\item Regional Consumption Weight
	\begin{align}
		\hat{C}_{1 2t} - \hat{C}_{1 1t} &= \hat{P}_{1t} - \hat{P}_{2t} \tag{\ref{eq_v6:reg-C-eta-12-t-ll}a} \\
		\hat{C}_{2 2t} - \hat{C}_{2 1t} &= \hat{P}_{1t} - \hat{P}_{2t} \tag{\ref{eq_v6:reg-C-eta-12-t-ll}b}
	\end{align}
	
	\item Regional Consumption of Good 1
	\begin{align}
		\hat{C}_{1 t} - \hat{C}_{1 1 t} &= (1 - \omega_{1 1}) (\hat{P}_{1t} - \hat{P}_{2t}) \tag{\ref{eq_v6:reg-C-eta-1-t-ll}a} \\
		\hat{C}_{2 t} - \hat{C}_{2 1 t} &= (1 - \omega_{2 1}) (\hat{P}_{1t} - \hat{P}_{2t}) \tag{\ref{eq_v6:reg-C-eta-1-t-ll}b}
	\end{align}
	...
	\item Labor Supply
	\begin{alignat}{2}
		\varphi \hat{L}_{1 t} + \sigma \hat{C}_{1 t} &= \hat{W}_{1 t} - \hat{Q}_{1t} \tag{\ref{eq_v6:reg-ll-labor-supply}a} \\
		\varphi \hat{L}_{2 t} + \sigma \hat{C}_{2 t} &= \hat{W}_{2 t} - \hat{Q}_{1t} \tag{\ref{eq_v6:reg-ll-labor-supply}b}
	\end{alignat}
	...
	\item Production Function
	\begin{align}
		\hat{Y}_{1 t} &= \hat{Z}_{A1 t} + \hat{L}_{1 t} \tag{\ref{eq_v6:reg-ll-final-goods-firm-bundle-rule-3}a} \\
		\hat{Y}_{2 t} &= \hat{Z}_{A2 t} + \hat{L}_{2 t} \tag{\ref{eq_v6:reg-ll-final-goods-firm-bundle-rule-3}b} 
	\end{align}
	
	\item Marginal Cost
	\begin{align}
		\hat{P}_{1 t} + \hat{\lambda}_{1 t} &= \hat{W}_{1 t} - \hat{Z}_{A1 t} \tag{\ref{eq_v6:reg-int-good-firm-FOC-Lt-ll}a} \\
		\hat{P}_{2 t} + \hat{\lambda}_{2 t} &= \hat{W}_{2 t} - \hat{Z}_{A2 t} \tag{\ref{eq_v6:reg-int-good-firm-FOC-Lt-ll}b}
	\end{align}
	...
	\item Productivity Shock
	\begin{alignat}{2}
		\hat{Z}_{A1 t} &= \rho_{A1} \hat{Z}_{A1, t-1} + \varepsilon_{A1} \tag{\ref{eq_v6:reg-ll-productivity-shock}a} \\
		\hat{Z}_{A2 t} &= \rho_{A2} \hat{Z}_{A2, t-1} + \varepsilon_{A2} \tag{\ref{eq_v6:reg-ll-productivity-shock}b}
	\end{alignat}
	...
	\item Regional Market Clearing Condition
	\begin{align}
		\hat{P}_{1 t} + \hat{Y}_{1 t} &= \hat{Q}_{1t} + \hat{C}_{1 t} \tag{\ref{eq_v6:reg-regional-demand-ll}a} \\
		\hat{P}_{2 t} + \hat{Y}_{2 t} &= \hat{Q}_{1t} + \hat{C}_{2 t} \tag{\ref{eq_v6:reg-regional-demand-ll}b}
	\end{align}
...
As it stands, the model has more forward-looking variables than eigenvalues greater than one, indicating that the model is indeterminate. To transform the model into one with a single solution, the number of eigenvalues and forward-looking variables must be equal. To achieve this, \textcite{farmer_solving_2015} substitutes the excess forward-looking variables with an expectational variable at time $t$, along with a expectational shock $sunspot_{\eta}$, representing the deviation between the expected and real values. For the present model, the variables created are the expected regional gross inflation rates $\pi_{\eta t}^{X}$:
\begin{align}
	\pi_{\eta t}^{X} &= \mathbb{E}_t \pi_{\eta, t+1} \label{eq_v6:reg-pix}\\
	sunspot_{\eta} &= \pi_{\eta t} - \pi_{\eta, t-1}^{X} \label{eq_v6:sunspot}
\end{align}
