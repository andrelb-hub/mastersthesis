...
\subsection{Regional New Keynesian Model}\label{sec_v6:nk-model}
	...
	\item a firm producing a final-goods, and
...
\subsubsection{Equilibrium Conditions}
	...
	\begin{align}
		...
		& \quad \text{where:} \quad P_{\eta t} Y_{\eta t} = 
		% P_{1t} C_{1 \eta t} + P_{2t} C_{2 \eta t} + B_{\eta t} = 
		Q_{\eta t} C_{\eta t} + B_{\eta t} \label{eq_v6:reg-regional-demand} \\
...
Due to the number of variables and equations that need to be solved, computational brute force will be necessary. \texttt{Dynare} is specialized software for macroeconomic modeling, commonly used for solving DSGE models. Before the model can be processed by the software, it must undergo linearization to eliminate the infinite sum in Equation \ref{eq_v6:reg-int-good-firm-optimal-price-FOC-3}. For this purpose, Uhlig's rules of log-linearization \cite{uhlig_toolkit_1999} will be applied to all equations in the model. For any given variable $X_{t}$, its deviation will be represented with a hat, $\hat{X}_{t}$.\footnote{see lemma \ref{lemma:uhligs-rules} for details.}
